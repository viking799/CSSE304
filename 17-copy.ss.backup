	...
	(apply-env env id
      ...
      [set!-exp (sym body)
        ...
        (begin
          ...
          (display sym)
          (display 'begin?)
          (display (2nd (eval-exp (var-exp sym) env)))
          (display 'begin!)
          (display (2nd (var-exp (2nd (eval-exp (var-exp sym) env))))) 
          (display 'tooooo!!!!!!!!!!)
          (display (if (expression? (eval-exp (syntax-expand body) env)) (eval-exp (eval-exp (syntax-expand body) env) env) (eval-exp (syntax-expand body) env) ))
          (display 'what!!!!!!!!!~~~~)
          (newline) (display 'damn!!!)
          (display (equal? (eval-exp (var-exp sym) env) (var-exp (2nd (eval-exp (var-exp sym) env)))))
          (newline)
        (set-box! (apply-env-ref
